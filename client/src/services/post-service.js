...
class PostService {
    ...
    getAllPosts = async () => {
        try {
            const res = await api(`${API_URL}/posts`);
    ...
    searchPosts = async (query) => {
        ...
        try {
            const res = await api(`${API_URL}/posts?query=${query}`);
            if (!res.success) throw new Error(res.message);
            this.posts = res.data;
            return { success: true, data: this.posts }
        } catch (err) {
    ...
    getUploadUrl = async () => {
        const res = await api(`${API_URL}/posts/upload/get-url`);
        return res;
    }
    ...
    savePost = async ({ imageUrl, title }) => {
        ...
        try {
            const res = await api(`${API_URL}/posts/upload`, body, 'POST');
            if (!res.success) throw new Error(res.message);
    ...
    likePost = async id => {
        ...
        try {
            const res = await api(`${API_URL}/posts/like/${id}`, null, 'PUT');
    ...
    unlikePost = async id => {
        const postIndex = this.posts.findIndex(post => post._id === id);
        try {
            const res = await api(`${API_URL}/posts/unlike/${id}`, null, 'PUT');
    ...
    deletePost = async id => {
        try {
            const res = await api(`${API_URL}/posts/${id}`, null, 'DELETE');
            if (!res.success) throw new Error(res.message);
